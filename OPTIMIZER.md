...
- **A matured verdict is actionable by itself.** Scoring a matured experiment justifies - and requires - that day's PR: a log-entry-only PR is a valid, expected output. Do not hold a verdict hostage to finding a content change to carry it; the do-nothing rule below applies only when nothing has matured AND nothing is actionable.
- **gsc verdicts come from committed milestones, never from CI inputs.** The public `audits/latest.json` slice excludes gsc by design (the tripwire in audit.yml), so a `gsc=` Reads-after can never be scored from input 1: score it from the newest committed `seo-reports/*.json` that contains a gsc block and postdates the maturity date. If none exists, write `gsc: awaiting local audit` as the verdict once and move on - it is owed to a future local run, not overdue daily.
...
- Branch: the harness-provided `optimize/...` branch. Title `optimize: <one line>`.
...
- **Commit through the signing tool, never raw git.** `git commit` in Bash produces an unsigned commit and fails review on sight. The harness checks out your working branch at job start (prefixed `optimize/`); edit files in the worktree, then commit every change with `mcp__github_file_ops__commit_files` (and deletions with `mcp__github_file_ops__delete_files`) - the API path GitHub signs, targeting that branch. Bash git stays for reading state: status, log, diff.
...
- Never merge, never push to main, never force-push, never open more than one PR per run.
- **A closed-unmerged PR is a verdict.** If a previous PR of yours was closed without merging, do not re-propose its change unless a human comment on that PR explicitly invites it (for example: closed for commit mechanics, content welcome to return). Absent such a comment, record the rejection in your log and move on.
- **An open PR of yours outranks new work.** If your previous PR is still open, process it before anything else: read its reviews and comments. If changes are requested, push the revisions to that same branch as your one action for the run, update your log entry in the same commit, and leave one short summary comment on the PR. If there is no new feedback, add nothing and exit.
- **PRs originate here only.** Agent PRs come from the scheduled run that tails the daily audit, or a manual workflow dispatch - never from issues (the @claude workflow answers on pull requests only). Feedback on your PRs is ordinary PR review: inline @claude replies handle interrogation and quick revisions in-thread; otherwise you process requested changes on your next scheduled run, or immediately when dispatched.
