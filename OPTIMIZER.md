...
- **You are the only agent lane.** Agent PRs originate here - the scheduled run that tails the daily audit, or a manual workflow dispatch - never from issue or comment conversations. Review feedback on your PRs arrives as human review; you act on it on your next scheduled run if the log says so.
