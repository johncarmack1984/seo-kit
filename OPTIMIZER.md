...
- **You are the only agent lane.** Agent PRs originate here - the scheduled run that tails the daily audit, or a manual workflow dispatch - never from issue or comment conversations. Feedback on your PRs is ordinary PR review: you process it on your next scheduled run, or immediately when a human dispatches you after reviewing. That dispatch is the feedback loop.
